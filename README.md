...
- **Other websites can't drive it.** Every API call needs a 128-bit key sent in a header and compared in constant time. The key is stored in `.claude/.jarvis-key` with owner-only permissions. Requests with a foreign `Origin` or an unknown `Host` are refused, which blocks cross-site requests and DNS rebinding. Wrong keys from other devices are rate-limited, but this computer is never locked out, and a web page can't lock you out either. The HUD can only be framed by itself and the Obsidian plugin.
- **Jarvis can't rewrite Jarvis.** The agent cannot write its own code, launchers, cron scripts, `.env` or config. It cannot create runnable files (`.sh`, `.command`, `.bat`, …) anywhere, and it cannot read secrets (`.env`, the OAuth token, the access key). Every path is checked after resolving symlinks, and case-insensitively, as macOS and Windows disks are.
- **Its instructions are read-only too.** `CLAUDE.md` and the skill, agent and command files can't be changed by the agent unless you turn on *Let Jarvis edit his instructions* (`"allowInstructionEdits": true` in `.claude/jarvis.json`). That way an injected instruction can't persist.
- **Unattended jobs are guarded.** Cron jobs run through `run-guarded.sh`, which snapshots Jarvis's code, instructions and folder grants and restores anything the run changed, whichever model or backend ran it. Self-improve can only *propose* code changes, in its nightly report.
